		...
		class Kirki_Installer_Control extends WP_Customize_Control {
			...
			public function render_content() {
				...
				<?php else : ?>
					...
					<p><?php printf( __( 'The Kirki Toolkit plugin is installed but not activated. Please <strong><a href="%s">activate it</a></strong> in order to take full advantage of the theme\'s options.', 'judgement' ), admin_url( 'plugins.php' ) ); ?></p>
				<?php
				endif;
			}
	...
	if ( !function_exists( 'kirki_installer_register' ) ) {
		...
		function kirki_installer_register( $wp_customize ) {
			...
			$wp_customize->add_section( 'kirki_installer', array(
				'title'			 => '',
				'description'	 => esc_attr__( 'If you want to take full advantage of the options this theme has to offer, please install the companion Kirki Toolkit plugin.', 'judgement' ),
				'priority'		 => -10,
